...
class PermissionedFormTest(TestCase):
    def test_unbound_form_without_user(self):
        ...
        self.assertInHTML(
            '<input type="text" name="last_name" required id="id_last_name">', form_html
        )
    ...
    def test_bound_form_without_user(self):
        ...
        form_html = form.as_p()
        self.assertInHTML(
            '<input type="text" name="first_name" value="David" required id="id_first_name">',
            form_html,
        )
        self.assertInHTML(
            '<input type="text" name="last_name" value="Bowie" required id="id_last_name">',
            form_html,
        )
    ...
    def test_bound_form_for_superuser(self):
        ...
        form = PersonForm(
            {"first_name": "David", "last_name": "Bowie"}, for_user=superuser
        )
        self.assertIn("first_name", form.fields)
        self.assertIn("last_name", form.fields)
        self.assertTrue(form.is_valid())
...
class PermissionedModelFormTest(TestCase):
    def setUp(self):
        self.country = Country.objects.create(
            name="Ukraine", description="A lovely country with a blue and yellow flag"
        )
        self.edit_permission = Permission.objects.get(
            codename="change_country_description"
        )
    ...
    def test_unbound_form_without_user(self):
        ...
        form_html = form.as_p()
        self.assertInHTML(
            '<input type="text" name="name" maxlength="255" value="Ukraine" required id="id_name">',
            form_html,
        )
        self.assertInHTML(
    ...
    def test_bound_form_without_user(self):
        form = CountryForm(
            {"name": "France", "description": "A lovely country with baguettes"},
            instance=self.country,
        )
        self.assertIn("name", form.fields)
        self.assertIn("description", form.fields)
        self.assertTrue(form.is_valid())
        ...
        self.assertEqual(
            form.cleaned_data["description"], "A lovely country with baguettes"
        )
        form.save()
    ...
    def test_bound_form_for_normal_user_without_permission(self):
        ...
        form = CountryForm(
            {
                "name": "Sweden",
                "description": "A lovely country with flatpack furniture",
            },
            instance=self.country,
            for_user=bob,
        )
        ...
        self.assertEqual(
            self.country.description, "A lovely country with a blue and yellow flag"
        )
...
class PermissionedClusterFormTest(TestCase):
    def setUp(self):
        self.page = Page.objects.create(
            title="Sheep", body="And did those sheep in ancient time"
        )
        self.page_data = {
            "title": "Teeth",
